...
describe("intent parsing",()=>{
  ...
  it("extracts brand and price limits",()=>{expect(parseCommand("Show Colgate toothpaste")).toMatchObject({intent:"SEARCH_PRODUCT",brand:"Colgate",normalizedItem:"toothpaste"});expect(parseCommand("100 rupaye ke andar toothpaste dikhao")).toMatchObject({intent:"SEARCH_PRODUCT",maxPrice:100})});
...
describe("existing domain and persistence behavior",()=>{
  ...
  it("ranks in-stock substitutes",()=>expect(rankSubstitutes(catalog.find(p=>p.id==="amul-taaza")!)[0].inStock).toBe(true));
  it("migrates old saved units and prices",()=>{const item={...makeItem("milk"),unit:"litre",estimatedUnitPricePaise:undefined,estimatedUnitPrice:56},data={...defaultData,list:[item]} as AppData;expect(loadData(serializeData(data)).list[0]).toMatchObject({unit:"l",estimatedUnitPricePaise:5600})});
