...
describe("Pantry removal and responsive reading order",()=>{
  ...
  it("keeps the speaker icon synced with the spoken-confirmation checkbox",()=>{expect(appSource).toContain('aria-pressed={data.speechEnabled}');expect(appSource.match(/data\.speechEnabled\?"🔊":"🔇"/g)).toHaveLength(2);expect(appSource).toContain('checked={data.speechEnabled}')});
});
