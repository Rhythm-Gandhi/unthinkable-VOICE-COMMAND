...
export function parseCommand(rawTranscript:string):ParsedCommand{
  ...
  if(has(text,["alternative","instead of","substitute","cheaper option","other brand","विकल्प","বিকল্প","વિકલ્પ","மாற்று","ప్రత్యామ్నాయం","بدل"])&&!(!first&&text.includes("cheaper option")))return{intent:"SHOW_SUBSTITUTES",...base,...first,confidence:first?.normalizedItem?.length?.9:.72};
  ...
  if(has(text,["find","search","show","dikhao","दिखाओ","খুঁজে","தேடு","చూపించు","ಹುಡುಕು","കാണിക്കുക","دکھاؤ"]))return{intent:"SEARCH_PRODUCT",...base,...first,maxPrice,brand:catalog.find(p=>text.includes(p.brand.toLowerCase()))?.brand,confidence:first?.normalizedItem?.length?.91:.55};
...
export function searchProducts(command:ParsedCommand){let rows=catalog.filter(p=>p.inStock);if(command.normalizedItem)rows=rows.filter(p=>p.normalizedName===command.normalizedItem||p.name.toLowerCase().includes(command.normalizedItem!));if(command.brand)rows=rows.filter(p=>p.brand.toLowerCase().includes(command.brand!.toLowerCase()));if(command.maxPrice!==undefined)rows=rows.filter(p=>p.price<=command.maxPrice!);return rows.sort((a,b)=>a.price-b.price)}
